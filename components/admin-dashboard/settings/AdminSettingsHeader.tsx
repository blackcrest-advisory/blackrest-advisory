...
export const AdminSettingsHeader = () => {
  return (
    <header className="relative overflow-hidden border border-border bg-card shadow-[var(--shadow-card)]">
      ...
      <div
        aria-hidden="true"
        className="pointer-events-none absolute right-0 top-0 h-full w-[42%] opacity-[0.24] [background-image:linear-gradient(to_right,var(--color-border)_1px,transparent_1px)] [background-size:54px_100%] [mask-image:linear-gradient(to_left,black,transparent)]"
      />

      <div className="relative z-10 grid gap-7 px-6 py-7 sm:px-7 lg:px-9 lg:py-8 xl:grid-cols-[minmax(0,1fr)_360px] xl:items-stretch">
        {/*===== TITLE =====*/}
        ...
        <div className="relative border border-secondary/15 bg-secondary/[0.035] px-5 py-5">
          <div className="flex items-start gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md border border-secondary/20 bg-secondary/[0.06] text-secondary">
...
function SecurityMeta({ label, value }: { label: string; value: string }) {
  return (
    <div className="px-3 first:pl-0 last:pr-0">
      ...
      <p className="mt-1 text-[11px] font-semibold text-heading">
        {value}
      </p>
    </div>
  );
